...
uint256 hashGenesisBlock("0x000008088ff886fdc9b2745efc3ebb828af6cb75014bd0727de7eb5d05bb144a"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // GoldenLoaf: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "GoldenLoaf Signed Message:\n";
...
{
    // GoldenLoaf: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1)
    {
       nSubsidy = 4000 * COIN; //1% premine + 3% IPO in 1 block
    }
    else if(nHeight > 20 && nHeight <= 6740)
    {
       nSubsidy = 1 * COIN;
    }
    else if(nHeight > 6740 && nHeight <= 35540)
    {
       nSubsidy = 0.5 * COIN;
    }
    else if(nHeight > 35540 && nHeight <= 350419)
    {
       nSubsidy = 0.25 * COIN;
    }
...
static const int64 nTargetSpacing = 90; // 1 blocks: 90 sec
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 960
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // GoldenLoaf: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, goldenloaf - DarkGravity v3, written by Evan Duffield - evan@goldenloaf.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // GoldenLoaf: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // GoldenLoaf: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x000005dbca1a52eea09eafd14d0c39a66146c13b7acc1a6803b77e5c8ab0d909");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Golden Loaf Yanukovich";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1404654450;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2613004;

        if (fTestNet)
        {
            block.nTime    = 1404649832;
            block.nNonce   = 1349403;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x296f319ec1e645923205064abf30b2a3aaa77bb55c49fc7523d3dce1a09745bb"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf6, 0xc6, 0xb6, 0xd6 }; // GoldenLoaf: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GoldenLoafMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GoldenLoafMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GoldenLoafMiner(CWallet *pwallet)
{
    printf("GoldenLoafMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("goldenloaf-miner");
    ...
    try { loop {
        ...
        printf("Running GoldenLoafMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GoldenLoafMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GoldenLoafMiner, pwallet));
}
